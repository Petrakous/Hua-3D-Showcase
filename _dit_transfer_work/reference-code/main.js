import { LOCATION_CATALOG } from "./viewer/sceneCatalog.js?v=20260704dit1";
import { PlayCanvasSogViewer } from "./viewer/playCanvasSogViewer.js?v=20260704cal4";
...
function applyCalibrationOverrideToAsset(asset) {
  ...
  if (!asset.streamingEnabled && asset.sceneCalibrationKey) {
    const savedTransforms = cloneStreamedTransformConfig(streamedTransformsOverrides[asset.sceneCalibrationKey]);
    if (savedTransforms?.scene) {
      asset = {
        ...asset,
        position: savedTransforms.scene.position,
        rotationDegrees: savedTransforms.scene.rotationDegrees,
        rotation: degreesToQuaternion(savedTransforms.scene.rotationDegrees),
        scale: savedTransforms.scene.scale,
      };
    }
    if (savedTransforms?.cameraStart) {
      asset = { ...asset, cameraStartOverride: savedTransforms.cameraStart };
    }
  }
...
calibrationSetCurrent.addEventListener("click", () => {
  ...
  sogViewer.setCameraStartMarkerVisible?.(true);
  calibrationShowCameraMarker.checked = true;
  populateCalibrationInputs(captured);
  setStatus("Camera captured", "Camera Start now matches the current view.");
});
