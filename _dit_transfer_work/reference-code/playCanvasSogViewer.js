...
class PlayCanvasSogViewer {
  ...
  drawEditorGuides(pc) {
    ...
    // Debug lines are submitted during update, after PlayCanvas has consumed the
    // current on-demand frame request. Keep the editor preview rendering while
    // guides are visible so camera changes and the newly queued lines are shown.
    this.app.renderNextFrame = true;
    const size = 24;
    const step = 2;
  ...
  _ensureManualBoxLabels() {
    ...
    for (const name of ["Left", "Right", "Bottom", "Top", "Back", "Front"]) {
  ...
  drawManualBoxPreview(pc) {
    ...
    // app.drawLine queues transient geometry; without another requested frame,
    // the viewer appears frozen until hiding the box triggers a final render.
    this.app.renderNextFrame = true;
  ...
  async load(asset, profile = { maxDpr: 1.05 }, onState) {
    ...
    app.on("update", (deltaSeconds) => {
      ...
      try {
        ...
        // Editor helpers must never interrupt the camera/model update loop.
